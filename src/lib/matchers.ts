...
import { NestApiError } from "../models/errors/nestapierror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | NestApiError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | NestApiError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new NestApiError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new NestApiError("API error occurred", {
          request,
          response,
          body,
        }),
